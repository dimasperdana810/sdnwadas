...
class jcore{
	...
	{
		$config = $this->config();
		$con= mysqli_connect($config['host'],$config['login'],$config['password'],$config['name']);
	//	$con = mysqli_connect("localhost","sipkinmy_iot","malhotra2303","sipkinmy_iot");
		return $con;
	...
	{
		$this->doConnect();
		$rs = mysqli_query($this->doConnect(),$sql);
		return $rs;
	}
